...
from utils.publisher import candidate_path_for, publish_candidate
...
class UpdateSource:
    def __init__(self):
        ...
        self.candidate_file = candidate_path_for(config.final_file)
    ...
    async def _start_aggregator(self, cache: dict):
        self.aggregator = ResultAggregator(
            ...
            write_interval=2.0,
            result=cache,
            output_path=self.candidate_file,
        )
    ...
    async def _stop_aggregator(self):
        if self.aggregator:
            ...
            return aggregator.result, aggregator.test_results
        return {}, {}
    ...
    # ----------------------------
    async def main(self):
        candidate = Path(self.candidate_file)
        source_report_candidate = Path(constants.source_report_candidate_path)
        try:
            if candidate.is_file():
                candidate.unlink()
            if source_report_candidate.is_file():
                source_report_candidate.unlink()
            ...
            if not self.channel_names:
                ...
                raise RuntimeError("no_channel_names")
            ...
            try:
                ...
                final_result, report_results = await self._stop_aggregator()

            publish_result = publish_candidate(
                self.candidate_file,
                final_path=config.final_file,
                report_path=constants.report_path,
                last_good_path=constants.last_good_result_path,
                template_path=config.source_file,
                channel_data=report_results or final_result,
                source_report_path=constants.source_report_candidate_path,
                min_coverage=config.publish_min_coverage,
                max_drop_ratio=config.publish_max_drop_ratio,
                critical_groups=config.publish_critical_groups,
            )
            if not publish_result.published:
                print(f"playlist publish rejected: {', '.join(publish_result.reasons)}", flush=True)
                return
            ...
            try:
                publish_candidate(
                    ...
                    max_drop_ratio=config.publish_max_drop_ratio,
                    critical_groups=config.publish_critical_groups,
                    unhandled_errors=[f"unhandled_exception:{type(exc).__name__}:{exc}"],
                )
            except Exception:
                pass
            print(f"update failed: {redact_text(str(exc))}", flush=True)
            raise
