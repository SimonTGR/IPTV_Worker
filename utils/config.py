...
class ConfigManager:
    ...
    def source_registry(self):
        return self.config.get("Settings", "source_registry", fallback="config/sources.json")

    @property
    def file_inbox_enabled(self):
        return self.config.getboolean("Settings", "file_inbox_enabled", fallback=True)
    ...
    def sort_by(self):
        raw = self.config.get(
            "Settings",
            "sort_by",
            fallback="playable,content_verified,stability,download_speed,bitrate,resolution,delay,source_priority",
        )
        allowed = (
    ...
    @property
    def open_filter_bitrate(self):
        return self.config.getboolean("Settings", "open_filter_bitrate", fallback=False)

    @property
    def min_bitrate_kbps(self):
        return self.config.getfloat("Settings", "min_bitrate_kbps", fallback=1000)

    @property
    def probe_attempts(self):
        return max(1, min(5, self.config.getint("Settings", "probe_attempts", fallback=2)))

    @property
    def min_success_ratio(self):
        return self.config.getfloat("Settings", "min_success_ratio", fallback=0.5)

    @property
    def max_consecutive_failures(self):
        return max(1, self.config.getint("Settings", "max_consecutive_failures", fallback=3))

    @property
    def open_content_verification(self):
        return self.config.getboolean("Settings", "open_content_verification", fallback=True)

    @property
    def content_probe_timeout(self):
        return self.config.getint("Settings", "content_probe_timeout", fallback=20)

    @property
    def content_probe_concurrency(self):
        return max(1, self.config.getint("Settings", "content_probe_concurrency", fallback=2))

    @property
    def untrusted_relay_domains(self):
        raw = self.config.get("Settings", "untrusted_relay_domains", fallback="live.catvod.com")
        return [value.strip().lower() for value in raw.split(",") if value.strip()]

    @property
    def publish_min_coverage(self):
        return max(0.0, min(1.0, self.config.getfloat("Settings", "publish_min_coverage", fallback=0.70)))

    @property
    def publish_max_drop_ratio(self):
        return max(0.0, min(1.0, self.config.getfloat("Settings", "publish_max_drop_ratio", fallback=0.30)))

    @property
    def publish_critical_groups(self):
        raw = self.config.get(
            "Settings", "publish_critical_groups", fallback="央视频道,广东频道,卫视频道"
        )
