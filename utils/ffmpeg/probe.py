...
def _parse_probe_data(data: dict) -> dict | None:
    """
    Parse ffprobe JSON and keep stream bitrate separate from download speed.
    """
    if not data:
        return None
    ...
    try:
        format_bitrate = (data.get('format') or {}).get('bit_rate')
        stream_bitrate = _safe_get(video, 'bit_rate')
        raw_bitrate = format_bitrate or stream_bitrate
        if raw_bitrate is not None:
            bitrate = float(raw_bitrate) / 1000.0
            if bitrate > 0:
