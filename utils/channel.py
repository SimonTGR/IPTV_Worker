...
from sources.registry import collect_configured_sources, refresh_configured_source_once
from utils.content_verifier import ContentVerifier, is_untrusted_relay
...
def _local_entry_url(entry) -> str:
    return entry.get("url", "") if isinstance(entry, dict) else entry


def format_channel_data(url: str | dict, origin: OriginType) -> ChannelData:
    """
    Format the channel data
    """
    metadata = dict(url) if isinstance(url, dict) else {}
    raw_url = metadata.get("url", "") if metadata else url
    url_partition = raw_url.partition("$")
    url = url_partition[0]
    ...
    for key in (
        "headers", "tvg_logo", "source_id", "source_type", "source_priority",
        "discovered_at", "source_path", "dynamic_base",
    ):
        if metadata.get(key) is not None:
            formatted[key] = metadata[key]
    formatted["untrusted_relay"] = is_untrusted_relay(url)
    return cast(ChannelData, formatted)
            ...
            if name_value and name_value[0]:
                ...
                if first_time:
                    ...
                    if open_local and local_data:
                        canonical_name = channel_alias.get_primary(name)
                        alias_names = set(channel_alias.get(canonical_name))
                        alias_names.update([name, format_name(name), canonical_name, format_name(canonical_name)])
                        for alias_name in alias_names:
                            if alias_name in local_data:
                                matched_local_names.add(alias_name)
                                for local_url in local_data[alias_name]:
                                    if not check_url_by_keywords(_local_entry_url(local_url), blacklist):
...
def get_channel_items(whitelist_maps, blacklist) -> CategoryChannelData:
    ...
    inbox_dir = os.path.abspath(os.path.join(constants.local_dir_path, "inbox"))
    archive_dir = os.path.abspath(os.path.join(constants.local_dir_path, "archive"))
    local_paths = [
        path for path in build_path_list(constants.local_dir_path)
        if not os.path.abspath(path).startswith((inbox_dir + os.sep, archive_dir + os.sep))
    ]
    local_data = get_name_urls_from_file([get_real_path(constants.local_path)] + local_paths)
    if config.file_inbox_enabled and os.path.exists(resource_path(config.source_registry)):
        try:
            configured = collect_configured_sources(
                config.source_registry,
                report_path=constants.source_report_candidate_path,
            )
            for source_name, source_items in configured.as_channel_map().items():
                existing_urls = {_local_entry_url(item) for item in local_data[source_name]}
                for item in source_items:
                    if item.get("url") not in existing_urls:
                        local_data[source_name].append(item)
                        existing_urls.add(item.get("url"))
        except Exception as e:
            print(f"source registry failed: {e}", flush=True)
        ...
        try:
            ...
            if normalized_url in existing_map:
                ...
                if existing_origin != "whitelist" and url_origin == "whitelist":
                    channel_list[existing_idx] = {**item,
                        "id": channel_id,
                        "url": normalized_url,
...
def is_valid_speed_result(info) -> bool:
    ...
    try:
        if info.get("playable") is False:
            ...
            return False
        if info.get("failure_reason") in {"wrong_content", "placeholder_fingerprint", "ad_or_no_signal"}:
            return False
        ...
        speed_val = info.get("download_speed_mbps", info.get("speed"))
        if speed_val is not None and (speed_val <= 0 or math.isinf(speed_val)):
            if not math.isinf(speed_val) and not info.get("failure_reason"):
                info["failure_reason"] = "speed_too_low"
            return False
        if open_filter_speed and speed_val is not None:
        ...
        if open_filter_resolution:
            ...
            if res_value < min_resolution_value:
                info["failure_reason"] = "resolution_too_low"
                return False
...
async def test_speed(data, ipv6=False, callback=None, on_task_complete=None):
    ...
    worker_refresh_tasks = {}
    worker_refresh_lock = asyncio.Lock()

    async def refresh_worker_once(source_id: str):
        async with worker_refresh_lock:
            task = worker_refresh_tasks.get(source_id)
            if task is None:
                task = asyncio.create_task(asyncio.to_thread(refresh_configured_source_once, source_id))
                worker_refresh_tasks[source_id] = task
        return await task
    ...
    async def limited_get_speed(channel_info):
        async with semaphore:
            ...
            for attempt in range(config.probe_attempts):
                if attempt:
                    invalidate_speed_cache(channel_info)
                attempts.append(await get_speed(
                    channel_info,
                    headers=headers,
            ...
            result = get_avg_result(attempts)
            transport_failed = result.get("delay_ms", result.get("delay")) in (None, -1) or not (
                result.get("download_speed_mbps", result.get("speed")) or 0
            )
            if transport_failed and channel_info.get("source_type") == "worker_discovery":
                refreshed = await refresh_worker_once(channel_info.get("source_id"))
            ...
            if transport_failed and is_untrusted_relay(channel_info.get("url", "")):
                result["failure_reason"] = "relay_token_expired"
            return result
    ...
    def _on_task_done(task):
        ...
        if is_valid:
            ...
            required_valid = urls_limit * 2 if config.open_content_verification else urls_limit
            if not open_full_speed_test and valid_count_by_channel[(cate, name)] >= required_valid:
            ...
            try:
                ...
                result_logger.info(
                    ...
                    f"{t('name.delay')}: {merged.get('delay_ms', merged.get('delay')) or -1} ms, "
                    f"download: {(merged.get('download_speed_mbps', merged.get('speed')) or 0):.2f} M/s, "
                    f"bitrate: {(merged.get('bitrate_kbps') or 0):.0f} kbps, stability: {(merged.get('success_ratio') or 0):.2f}, "
    ...
    apply_stability_history(grouped_results)
    await ContentVerifier().verify_grouped_results(grouped_results)
...
def generate_channel_statistic(logger, cate, name, values):
    ...
    max_bitrate = max((v.get("bitrate_kbps") or 0 for v in valid_items), default=0)
    verified_content = len([v for v in valid_items if v.get("content_verified") is True])
    max_resolution = max(
...
def write_channel_to_file(data, ipv6=False, first_channel_name=None, skip_print=False, is_last=False,
                          final_path=None, include_auxiliary=True):
    """
    ...
    try:
        ...
        file_list = [{"path": final_path or config.final_file, "enable_log": True}]
        if include_auxiliary:
            file_list += [
                {"path": constants.ipv4_result_path, "ipv_type_prefer": ["ipv4"]},
                {"path": constants.ipv6_result_path, "ipv_type_prefer": ["ipv6"]},
            ]
        if include_auxiliary and config.open_rtmp and not os.getenv("GITHUB_ACTIONS"):
            file_list += [
